// controllers/entiteController.js
'use strict';
...
module.exports = {
  ...
  async list({ listOpts, filters }) {
    ...
    let sql = `
      SELECT e.*, r.nom AS region_nom, p.nom AS province_nom, c.nom AS commune_nom
      FROM ${TABLE} e
    ...
    if (SOFT_DELETE) {
      where.push(`e.${COL_DELETED_AT} IS NULL`);
    }

    // Recherche
    if (search?.q) {
      const like = `%${search.q}%`;
      const conds = SEARCH_FIELDS.map(f => `e.${f} LIKE ?`).join(' OR ');
      where.push(`(${conds})`);
    ...
    if (sortBy && SORTABLE_FIELDS.includes(sortBy)) {
      sql += ` ORDER BY e.${sortBy} ${sortDir === 'desc' ? 'DESC' : 'ASC'}`;
    } else {
    ...
    if (limit) {
      let countSql = `SELECT COUNT(*) as cnt FROM ${TABLE} e`;
      const countParams = [];
      if (SOFT_DELETE) {
        countSql += ` WHERE e.${COL_DELETED_AT} IS NULL`;
      }
      if (search?.q) {
        const like = `%${search.q}%`;
        const conds = SEARCH_FIELDS.map(f => `e.${f} LIKE ?`).join(' OR ');
  ...
  async get({ id }) {
    const sql = `
      ...
      LEFT JOIN provinces p ON e.province_id = p.id
      LEFT JOIN communes c ON e.commune_id = c.id
      WHERE e.id = ? ${SOFT_DELETE ? `AND e.${COL_DELETED_AT} IS NULL` : ''}
    `;
   ...
   */
  async add({ body }) {
    const { nom, code, region_id, province_id, commune_id, localite_id } = body;
    if (!nom || !code) {
    ...
    const now = nowISO();
    const sql = `
      INSERT INTO ${TABLE} (nom, code, region_id, province_id, commune_id, localite_id, ${COL_CREATED_AT}, ${COL_UPDATED_AT})
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  /**
   * Mettre à jour une entité (PUT)
   */
  async update({ id, body }) {
    const existing = await this.get({ id });
    if (!existing) return null;

    const { nom, code, region_id, province_id, commune_id } = body;
    const sql = `
      UPDATE ${TABLE}
      SET nom = ?, code = ?, region_id = ?, province_id = ?, commune_id = ?, ${COL_UPDATED_AT} = ?
      WHERE id = ?
    `;
    await db.run(sql, [
      nom ?? existing.nom,
      ...
      commune_id ?? existing.commune_id,
      nowISO(),
      id
    ]);
  ...
  async bulkAdd({ items }) {
    const now = nowISO();
    const sql = `
      INSERT INTO ${TABLE} (nom, code, region_id, province_id, commune_id, ${COL_CREATED_AT}, ${COL_UPDATED_AT})
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
