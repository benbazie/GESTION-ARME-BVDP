// controllers/localiteController.js
'use strict';
...
module.exports = {
  ...
  async list({ listOpts, filters }) {
    ...
    let sql = `
      SELECT l.*, c.nom AS commune_nom, p.nom AS province_nom, r.nom AS region_nom
      FROM ${TABLE} l
   ...
   */
  async add({ body }) {
    const { nom, code, commune_id } = body;
    if (!nom || !code) {
    ...
    const now = nowISO();
    const sql = `
      INSERT INTO ${TABLE} (nom, code, commune_id, ${COL_CREATED_AT}, ${COL_UPDATED_AT})
      VALUES (?, ?, ?, ?, ?)
    `;
  ...
  /**
   * Mettre à jour une localité (PUT)
   */
  async update({ id, body }) {
    ...
    const sql = `
      UPDATE ${TABLE}
      SET nom = ?, code = ?, commune_id = ?, ${COL_UPDATED_AT} = ?
      WHERE id = ?
    `;
    await db.run(sql, [
      nom ?? existing.nom,
