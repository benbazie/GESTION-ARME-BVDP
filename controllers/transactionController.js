...
const SEARCH_FIELDS = ['type', 'reference', 'description'];
const SORTABLE_FIELDS = ['id', 'date_transaction', 'created_at'];
...
module.exports = {
  ...
  async get({ id }) {
    return db.get(`SELECT * FROM ${TABLE} WHERE id = ? ${SOFT_DELETE ? `AND ${COL_DELETED_AT} IS NULL` : ''}`, [id]);
  },

  async add({ body }) {
    const { type, reference, description, date_transaction } = body;
    if (!type || !reference) throw new Error('Champs "type" et "reference" obligatoires');

    const now = nowISO();
    const result = await db.run(
      `INSERT INTO ${TABLE} (type, reference, description, date_transaction, ${COL_CREATED_AT}, ${COL_UPDATED_AT})
       VALUES (?, ?, ?, ?, ?, ?)`,
      [type, reference, description || '', date_transaction || now, now, now]
    );
  ...
  async update({ id, body }) {
    ...
    await db.run(
      `UPDATE ${TABLE}
       SET type = ?, reference = ?, description = ?, date_transaction = ?, ${COL_UPDATED_AT} = ?
       WHERE id = ?`,
      [
        type ?? existing.type,
        reference ?? existing.reference,
        description ?? existing.description,
        date_transaction ?? existing.date_transaction,
        nowISO(),
        id
      ]
    );
