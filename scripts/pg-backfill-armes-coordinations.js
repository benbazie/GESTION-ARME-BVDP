...
async function main() {
  ...
  try {
    ...
    const q1 = await client.query(`
      ...
      WHERE a.coordination_id = c.id
        AND c.type = 'communale'
        AND (
          a.coordination_communale_id IS NULL
          OR a.coordination_provinciale_id IS NULL
          OR a.coordination_regionale_id IS NULL
        )
    `)
    ...
    const q2 = await client.query(`
      ...
      WHERE a.coordination_id = c.id
        ...
        AND (
          a.coordination_provinciale_id IS NULL
          OR a.coordination_regionale_id IS NULL
        )
    `)
    ...
    const q3 = await client.query(`
      ...
      WHERE a.coordination_id = c.id
        AND c.type = 'regional'
        AND a.coordination_regionale_id IS NULL
    `)

    await client.query('COMMIT')

    console.log('🎯 Backfill armes terminé.')
    ...
    console.error('❌ Backfill armes échoué', err)
    process.exit(1)
  } finally {
    client.release()
    await pool.end()
