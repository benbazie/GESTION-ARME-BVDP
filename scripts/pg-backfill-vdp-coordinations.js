...
async function main() {
  ...
  try {
    ...
    // 1) Communale -> fill communale + provinciale + regionale via parents
    const q1 = await client.query(`
      ...
      WHERE v.coordination_id = c.id
        AND c.type = 'communale'
        AND (
          v.coordination_communale_id IS NULL
          OR v.coordination_provinciale_id IS NULL
          OR v.coordination_regionale_id IS NULL
        )
    `)

    // 2) Provincial -> fill provinciale + regionale via parent
    const q2 = await client.query(`
      ...
      WHERE v.coordination_id = c.id
        ...
        AND (
          v.coordination_provinciale_id IS NULL
          OR v.coordination_regionale_id IS NULL
        )
    `)

    // 3) Regional -> fill regionale by region (and entite when possible)
    const q3 = await client.query(`
      ...
      WHERE v.coordination_id = c.id
        AND c.type = 'regional'
        AND v.coordination_regionale_id IS NULL
    `)
