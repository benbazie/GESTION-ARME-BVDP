// src/components/DotationMunitionForm.jsx
...
import './DotationMunitionForm.css';

const { Option } = Select;

export default function DotationMunitionForm() {
  ...
  // Charger sous-entités / coordinations
  const handleEntityChange = async entiteId => {
    try {
      const data = await window.api.call('getSubEntitiesByEntite', { entiteId });
  ...
  const onFinish = async values => {
    ...
    try {
      ...
      values.ressource_type = 'munition';
      ...
      navigate('/dashboard/dotation-munition');
    } catch {
  ...
  // Observateur du type de dotation
  const typeDot = Form.useWatch('type_dotation', form);

  return (
    <div className="dotation-munition-form">
      <Card
        title={id ? 'Modifier Dotation Munition' : 'Créer Dotation Munition'}
        className="form-card"
      >
        <Form
          form={form}
        ...
        >
          ...
          <Form.Item
            name="ressource_id"
            label="ID de la Munition"
            rules={[{ required: true }]}
          >
            <Input placeholder="ID de la munition dotée" />
          </Form.Item>
          ...
          <Form.Item name="observation" label="Observation">
            <Input.TextArea rows={3} placeholder="Observation (facultatif)" />
          </Form.Item>
          ...
          <Form.Item>
            <Space>
              <Button
                ...
                onClick={() => navigate("/dashboard/dotation-munition")}
              >
                Retour
              </Button>
              ...
              <Button onClick={() => navigate('/dotations/munition')}>
                Annuler
              </Button>
            </Space>
          </Form.Item>
