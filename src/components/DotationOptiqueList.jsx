// src/components/DotationOptiqueList.jsx
...
import './DotationOptiqueList.css';

export default function DotationOptiqueList() {
  const [dotations, setDotations] = useState([]);
  const [loading, setLoading] = useState(false);
  const [filterForm] = Form.useForm();
  const navigate = useNavigate();

  // 1. Charger toutes les dotations puis filtrer sur optique
    ...
    try {
      ...
      setDotations(
        (Array.isArray(data) ? data : []).filter(
          item => item.ressource_type === 'optique'
        )
      );
    } catch {
      message.error("Erreur lors du chargement des dotations d'optiques.");
    } finally {
      setLoading(false);
    }
  };
        ...
        record.vdp_id && record.vdp ? (
          <div className="vdp-reference">
            ...
            <div className="vdp-details">
              {record.vdp.entite && `Entité : ${record.vdp.entite}`}<br />
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button
            icon={<EditOutlined />}
            onClick={() =>
              navigate(`/dotations/optique/form/${record.id}`)
            }
          >
            Modifier
          </Button>
  ...
  return (
    <div className="dotation-optique-list">
      <Card
        title="Filtrer les dotations d'optiques"
          ...
          <Form.Item>
            <Space>
              ...
              <Button
                type="primary"
                icon={<PlusOutlined />}
                onClick={() => navigate("/dashboard/dotation-optique/add")}
              >
      ...
      <Card
        title="Liste des dotations d'optiques"
        className="list-card"
      >
        <Table
          columns={columns}
