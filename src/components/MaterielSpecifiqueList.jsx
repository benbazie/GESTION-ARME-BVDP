// src/components/MaterielSpecifiqueList.js
...
const DIMENSION_DEFINITIONS = {
  ...
  categorie: { label: "Répartition par catégorie", accessor: row => row.categorie || "Non renseigné" },
  etat: { label: "Répartition par état", accessor: row => row.etat || "Indéfini" },
};

function MaterielSpecifiqueList() {
  ...
  const enrichedData = useMemo(
    () =>
      rawData.map((row, index) => {
        const cfg = configMap.get(String(row.config_materiel_id)) || {};
        return {
          ...row,
          __rowNumber: index + 1,
  ...
  const tableColumns = useMemo(() => {
        ...
        switch (key) {
          ...
          case "etat":
            return {
              ...
              key: "etat",
              width: 130,
              render: value => <Tag color={value === "En service" ? "green" : "geekblue"}>{value || "—"}</Tag>,
            };
          case "entite":
            return {
              title: "Entité",
    ...
    base.push({
      ...
      render: (_, record) => (
        <Space>
          <Button size="small" onClick={() => navigate(`/dashboard/materiel/edit/${record.id}`)}>
            Modifier
          </Button>
          <Button size="small" onClick={() => navigate(`/dashboard/dotation-materiel?materiel=${record.id}`)}>
            Historique
          </Button>
        </Space>
      ),
  ...
  const fetchConfigs = useCallback(async () => {
    const data = await safeCall(["getConfigMaterielList", "getConfigMateriels", "getConfigMateriel"]).catch(() => []);
    const arr = Array.isArray(data) ? data : (data && Array.isArray(data.rows) ? data.rows : []);
    setConfigs(arr);
  }, []);

  const fetchEntites = useCallback(async () => {
    const data = await safeCall(["getEntitesList", "getEntites"]).catch(() => []);
    setEntites(Array.isArray(data) ? data : data?.rows || []);
  }, []);

  const fetchRegions = useCallback(async () => {
    const data = await safeCall(["getRegionsList", "getRegions"]).catch(() => []);
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      let list = await safeCall(
        ["getMaterielsSpecifiquesList", "getMaterielsSpecifiques", "getMaterielSpecifiqueList"],
        params
      );
      let array = normalizeArray(list);

      if (!array.length) {
        const httpList = await api.getMaterielsSpecifiquesList(params).catch(() => []);
        array = normalizeArray(httpList);
      }

      if (!array.length) {
        const restList = await fetchJson("/api/materiels_specifiques", params).catch(() => []);
        array = normalizeArray(restList);
      }

      if (!array.length) {
        message.warning("Aucun matériel spécifique trouvé dans la base.");
      }
  ...
  const getDocumentTitle = useCallback(() => {
    ...
    const parts = ["Inventaire des matériels spécifiques"];
    if (filters.type) parts.push(`Type: ${filters.type}`);
    if (filters.categorie) parts.push(`Catégorie: ${filters.categorie}`);
    if (filters.etat) parts.push(`État: ${filters.etat}`);
  ...
  const exportToExcel = (dataset = filtered) => {
    ...
    link.href = url;
    link.download = "materiels_specifiques.xls";
  ...
  const exportToWord = (dataset = filtered) => {
    ...
    Packer.toBlob(doc)
      .then(blob => saveAs(blob, "materiels_specifiques.docx"))
  ...
  const stats = useMemo(
    () => ({
      ...
      enService: filtered.filter(item => item.etat === "En service").length,
      enMagasin: filtered.filter(item => item.etat === "En magasin").length,
    }),
    [filtered]
  );
    ...
    <div className="munition-list-page">
      ...
      <Card className="munition-list-shell" variant="borderless">
        <div className="munition-list-header">
          <div>
            <Title level={3} className="munition-list-title">Gestion des matériels spécifiques</Title>
            <Text className="munition-list-subtitle">
              Filtrez, analysez et exportez vos matériels en temps réel, avec les mêmes outils que le module munitions.
            </Text>
          </div>
          <Space wrap>
            <Button
            ...
            <Row gutter={12}>
              <Col xs={24} md={8}>
                <Form.Item name="period" noStyle>
                  <RangePicker
                    ...
                    onChange={value => setFilterField("period", value)}
                    style={{ width: "100%" }}
                    format="DD/MM/YYYY"
                  />
                </Form.Item>
              ...
              <Col xs={24} md={4}>
                <Select
                  ...
                  onChange={value => setFilterField("categorie", value)}
                  style={{ width: "100%" }}
                >
              ...
              <Col xs={24} md={4}>
                <Select
                  ...
                  placeholder="État"
                  value={filters.etat}
                  onChange={value => setFilterField("etat", value)}
                  style={{ width: "100%" }}
                >
              ...
              <Col xs={24} md={4}>
                <Select
                  ...
                  onChange={value => setFilterField("entite_id", value)}
                  style={{ width: "100%" }}
                  showSearch
        ...
        <div className="munition-list-reset-bar">
          <Space>
            ...
            <Text strong>Total : {stats.total} — En service : {stats.enService} — En magasin : {stats.enMagasin}</Text>
          </Space>
        </div>
        ...
        <Card className="munition-list-preview-card" size="small" variant="borderless">
          <Space direction="vertical" size="middle" style={{ width: "100%" }}>
            ...
            <Text type="secondary">
              Préparez impression et exports avec exactement les mêmes colonnes que le tableau principal.
            </Text>
            <Space wrap>
            ...
            <Row gutter={[12, 12]} justify="space-between" align="middle">
              ...
              <Col xs={24} lg={14}>
                <Space wrap>
                  ...
                  <Button type="primary" icon={<PrinterOutlined />} onClick={() => handlePrint(previewData)}>
                    Imprimer
                  </Button>
                  <Button icon={<FileExcelOutlined />} onClick={() => exportToExcel(previewData)}>
                    Export Excel
                  </Button>
                  <Button icon={<FileWordOutlined />} onClick={() => exportToWord(previewData)}>
                    Export Word
                  </Button>
                </Space>
              </Col>
...
export default MaterielSpecifiqueList;
